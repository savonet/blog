...
If you are lazy, here's the most key information in this post:
...
to measure the application's memory use and separate shared memory from private memory.
...
Finally, let's explore optimizing memory usage through the _script caching_ feature, which is available starting with Liquidsoap version `2.3.0`.
