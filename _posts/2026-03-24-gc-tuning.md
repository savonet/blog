...
This post explores how to tune three interacting parameters — `settings.frame.duration`, `runtime.gc` `minor_heap_size`, and `runtime.gc` `space_overhead` — to reduce both CPU and memory usage for a typical streaming script. Spoiler: the absolute numbers are modest enough that changing the global defaults is not warranted, but per-script tuning can be worthwhile if you are running many instances.
...
### 20-minute confirmation

The 20-minute matrix runs are enough to see the trend, but not long enough to let memory stabilise — startup and warm-up effects dominate the early samples and can bias the averages. We ran a dedicated 50-minute three-way comparison (baseline, top-1, top-2) to get a cleaner picture.
...
That said, the numbers are script-specific. A script doing heavy DSP, video processing, or complex routing will have very different allocation patterns. The default OCaml GC settings are deliberately conservative and work well across a wide range of workloads. We would not recommend changing them globally. But for a known, stable, production script running at scale, spending an hour with the test harness above is worth it.

The general principle transfers broadly: figure out your dominant allocation (for Liquidsoap, it is almost always the audio frame buffer), size your minor heap to hold it comfortably (roughly 2–3×), and tune `space_overhead` downward until the memory savings are no longer worth the marginal GC cost.
